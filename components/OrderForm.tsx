...
export default function OrderForm() {
  ...
  // Calculate totals in SEK (surcharge applies to package, not express)
  const baseSEK =
    selectedPaket === '2-jackor'
      ? BASE_PRICES.double + locale.surcharge
      : BASE_PRICES.single + locale.surcharge
  const totalSEK = expressChecked ? baseSEK + BASE_PRICES.express : baseSEK

  // Formatted display values
  const singleFormatted = formatPrice(BASE_PRICES.single + locale.surcharge, locale)
  const doubleFormatted = formatPrice(BASE_PRICES.double + locale.surcharge, locale)
  const expressFormatted = formatPrice(BASE_PRICES.express, locale)
  const totalFormatted = formatPrice(totalSEK, locale)

  // Postal code validation pattern: strict Swedish format for SE, flexible for others
  const postnummerPattern =
    locale.country === 'SE'
      ? {
          value: /^\d{3}\s?\d{2}$/,
  ...
  const postnummerPlaceholder =
    locale.country === 'SE' ? '114 29' : '1234'
  ...
  const postnordText =
    locale.country === 'SE' ? 'Fri frakt tur & retur med PostNord' : 'Fri frakt tur & retur via närmaste postombud'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <div className="lg:sticky lg:top-12">
            ...
            <h2 className="section-title mb-6">Kom igång på tre minuter</h2>
            <p className="font-sans text-gray-500 leading-relaxed mb-8">
              Fyll i formuläret så skickar vi påsen till din adress. I påsen
              finns allt du behöver – inklusive returetikett och instruktioner.
            </p>

            {/* What's included */}
            <div className="bg-white p-6 border border-gray-200 mb-8">
              <h3 className="font-serif text-base text-forest-green mb-4">
                Ingår i priset
              </h3>
              <ul className="space-y-2.5">
                {[
                  ...
                  <li
                    key={item}
                    className="flex items-start gap-2.5 text-sm text-gray-600 font-sans"
                  >
                    <svg
              ...
              {locale.surcharge > 0 && (
                <p className="mt-4 text-xs text-gray-400 font-sans border-t border-gray-100 pt-3">
                  Frakttillägg för internationell leverans ingår
                </p>
              )}

              {/* Delivery time */}
              <p className="mt-3 text-xs text-gray-400 font-sans">
                Leveranstid: {locale.deliveryDays}
              </p>
            </div>
            ...
            <div className="bg-forest-green p-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-serif text-cream text-sm">
                  {selectedPaket === '2-jackor' ? doubleFormatted : singleFormatted}
                </span>
              </div>
              {expressChecked && (
                <div className="flex justify-between items-center mb-2">
          ...
          <div>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`form-input ${
                      errors.efternamn ? 'border-red-400' : ''
                    }`}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    className={`form-input ${
                      errors.postnummer ? 'border-red-400' : ''
                    }`}
                    placeholder={postnummerPlaceholder}
                    {...register('postnummer', {
                      required: 'Postnummer är obligatoriskt',
                      pattern: postnummerPattern,
                    })}
                  />
                  {errors.postnummer && (
                    <p className="mt-1.5 text-xs text-red-500 font-sans">
                  ...
                  {[
                    {
                      value: '1-jacka',
                      label: '1 jacka',
                      priceDisplay: singleFormatted,
                    },
                    {
                      value: '2-jackor',
                      label: '2 jackor',
                      priceDisplay: doubleFormatted,
                      badge: 'Populärast',
                    },
                  ].map((option) => (
                    <label
                    ...
                    >
                      ...
                      <span className="font-serif text-sm text-forest-green whitespace-nowrap">
                        {option.priceDisplay}
                      </span>
                    </label>
                  ))}
              ...
              <div>
                <label
                  className={`flex items-start gap-3 p-4 border cursor-pointer transition-colors duration-150 ${
                    expressChecked
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-sans text-xs text-gray-400 mt-0.5">
                      Klar inom 1 vecka istället för {locale.deliveryDays}
                    </p>
                  </div>
                </label>
              </div>
              ...
              <div>
                ...
                {errors.villkor && (
                  <p className="mt-1.5 text-xs text-red-500 font-sans ml-7">
                    {errors.villkor.message}
                  </p>
                )}
              </div>
